...
{
    int one = 0, two = 0, three = 0;//which means repeat 1, 2, 3 times respectively
    for (int i = 0; i < numsSize; i++) {
        int lastone = one;
        int lasttwo = two;
        one = ~three & (one ^ nums[i]);
        two = (~nums[i]&~three&two) | (~three&two&~lastone) | (nums[i]&~three&~two&lastone);
        three = (~nums[i]&three&~lasttwo&~lastone)|(nums[i]&~three&lasttwo&lastone);
    }
    return one;
}
